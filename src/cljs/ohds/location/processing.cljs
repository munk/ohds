...
(extend-protocol Message m/ChangeLocationName
...
(extend-protocol Message m/ChangeLocationHierarchy
  (process-message [response app]
    (let [uuid (:hierarchy response)]
      (assoc app :location-hierarchy uuid))))

(extend-protocol EventSource m/LocationHierarchyResults
    (watch-channels [_ _]
      #{(backend/locations)}))

(extend-protocol Message m/LocationHierarchyResults
  (process-message [response app]
    (let [{status :status
           body :body} response]
      (case status
...
(extend-protocol EventSource m/ChangeLocationHierarchy
  (watch-channels [response app]
    (let [{uuid :hierarchy} response]
      #{(backend/locations uuid)})))

(extend-protocol Message m/LocationResults
  (process-message [response app]
    (let [{status :status
           body :body} response]
      (case status
...
(extend-protocol Message m/ChangeLocation
  (process-message [response app]
